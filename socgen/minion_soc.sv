...
module minion_soc (
  ...
  inout wire       sd_cmd,
  output reg       sd_reset ,                        inout wire PS2_CLK,
  inout wire PS2_DATA  ,                       output wire       VGA_HS_O,
...
wire        uart_almostfull_1;
wire        uart_full_1;
wire        uart_rderr_1;
wire        uart_wrerr_1;
wire        uart_empty_1;

wire [11:0] uart_wrcount_1;
...
uart i_uart_1 (
  ...
  .rx(uart_maj_1), // Incoming serial line
  .tx(uart_tx_1), // Outgoing serial line
...
my_fifo #(.width(9)) uart_rx_fifo_1 (
  ...
  .rdcount(uart_rdcount_1),         // 12-bit output: Read count
  .rderr(uart_rderr_1),             // 1-bit output: Read error
  .wrcount(uart_wrcount_1),         // 12-bit output: Write count
  .wrerr(uart_wrerr_1),             // 1-bit output: Write error
  .almostfull(uart_almostfull_1),   // output wire almost full
  .full(uart_full_1),    // output wire full
...
always @(posedge msoc_clk or negedge rstn)
  ...
  if (core_lsu_req&core_lsu_we&one_hot_data_addr[2])
    case(core_lsu_addr[5:2])
      0: begin
          u_trans_1 <= 1'b1;
          u_tx_byte_1 <= core_lsu_wdata[7:0];
        end
